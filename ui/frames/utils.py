...
try:
    ...
    _CALENDAR_PROBE_ERROR: Exception | None = None
except Exception as exc:  # pragma: no cover - only exercised when dependency is missing
    DateEntry = None  # type: ignore[assignment]
    _CALENDAR_PROBE_ERROR = exc

_CALENDAR_WARNING_EMITTED = False
...
def create_date_entry(
    ...
    **kwargs,
    ...
    if DateEntry is None:
        _warn_calendar_fallback("tkcalendar no disponible", _CALENDAR_PROBE_ERROR)
    ...
    except Exception as exc:  # pragma: no cover - fallback only when calendar fails
        _warn_calendar_fallback("No se pudo inicializar tkcalendar", exc)
        return _build_plain_entry(parent, textvariable, options)
...
def _warn_calendar_fallback(context: str, exc: Exception | None) -> None:
    global _CALENDAR_WARNING_EMITTED

    if _CALENDAR_WARNING_EMITTED:
        return

    _CALENDAR_WARNING_EMITTED = True
    details = f" ({exc})" if exc else ""
    sys.stderr.write(f"[date-entry] Recurriendo a entrada simple: {context}{details}\n")
...
def create_scrollable_container(
    ...
    _scrollregion_job = None

    def _configure_canvas(_event):
        nonlocal _scrollregion_job
        if _scrollregion_job:
            try:
                canvas.after_cancel(_scrollregion_job)
            except Exception:
                pass
        try:
            _scrollregion_job = canvas.after(50, lambda: canvas.configure(scrollregion=canvas.bbox("all")))
        except Exception:
...
def resize_scrollable_to_content(
    container: Any,
    *,
    ...
    """Sincroniza el ``scrollregion`` del canvas con su contenido."""
    ...
    def _perform_resize():
        ...
        if scrollregion:
            try:
                if scrollregion != getattr(container, "_last_scrollregion", None):
                    canvas.configure(scrollregion=scrollregion)
                    setattr(container, "_last_scrollregion", scrollregion)
    ...
    delay = max(0, debounce_ms or 0)
    scheduler = getattr(container, "after", None)
    if callable(scheduler):
        try:
            job_id = scheduler(delay, _perform_resize)
            setattr(container, "_resize_scroll_job", job_id)
            return
        except Exception:
            pass

    scheduler_idle = getattr(container, "after_idle", None)
    if callable(scheduler_idle):
        try:
            job_id = scheduler_idle(_perform_resize)
            setattr(container, "_resize_scroll_job", job_id)
            return
        except Exception:
            pass
