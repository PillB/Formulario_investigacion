...
class TeamMemberFrame:
        ...
        for widget, label, var, badge_key in [
            ...
            self.validators.append(
                FieldValidator(
                    ...
                    self.badges.wrap_validation(
                        badge_key,
                        lambda v=var, label_text=label: self._validate_date_field(
                            v, label_text
                        ),
                    ),
                    self.logs,
        ...
        for widget, variable, catalog, label, badge_key in catalog_validations:
            self.validators.append(
                FieldValidator(
                    ...
                    self.badges.wrap_validation(
                        badge_key,
                        lambda v=variable, c=catalog, label_text=label: self._validate_catalog_selection(
                            v.get(), label_text, c
                        ),
                    ),
                    self.logs,
    ...
    def _bind_date_validation(self, widget, variable: tk.StringVar, label: str) -> None:
        widget.bind(
            "<FocusOut>",
            lambda _e, v=variable, label_text=label: self._validate_date_field(
                v, label_text, show_alert=True
            ),
            add="+",
        )
