...
- Orden único para **Productos** y CSV combinado: `id_producto`, `id_cliente`, `tipo_producto`, `categoria1`, `categoria2`, `modalidad`, `canal`, `proceso`, `fecha_ocurrencia`, `fecha_descubrimiento`, `monto_investigado`, `tipo_moneda`, `monto_perdida_fraude`, `monto_falla_procesos`, `monto_contingencia`, `monto_recuperado`, `monto_pago_deuda`, `id_reclamo`, `nombre_analitica`, `codigo_analitica` (y `id_colaborador` al final para `datos_combinados_masivos.csv`). Las tres columnas de reclamos siempre van al final en el mismo orden que los encabezados esperados.
