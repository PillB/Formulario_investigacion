...
class FraudCaseApp:
    ...
    def validate_data(self):
        ...
        risk_ids = set()
        plan_ids = set()
        risk_lookup = getattr(self, "risk_lookup", {}) or {}
        for idx, r in enumerate(self.risk_frames, start=1):
            ...
            if rid:
                risk_ids.add(rid)
            if is_catalog_mode and risk_lookup and rid and rid not in risk_lookup:
                errors.append(
                    f"Riesgo {idx}: El ID de riesgo {rid} no existe en el catálogo."
                )
            if is_catalog_mode:
