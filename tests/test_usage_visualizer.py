...
def test_visualize_usage_builds_heatmaps(sample_log_file, tmp_path):
    ...
    heatmap_datasets = _build_heatmap_dataset(
        load_log_rows(sample_log_file),
        DEFAULT_SCREEN_HINTS,
        (400, 300),
    )
    assert heatmap_datasets
    assert heatmap_datasets[0].coords
    assert len(report.figure.axes[0].images) > 0
